...
try:
    while(i>1):
        historicParam={ "exchange": "NSE","symboltoken": nifty_100_stocks[i-2],"interval": "FIVE_MINUTE","fromdate": "2021-02-08 09:21", "todate": "2021-02-08 09:25"}
        element=obj.getCandleData(historicParam)['data']
        prices.range('c'+str(i)).value=element['open']
        prices.range('d'+str(i)).value=element['high']
        prices.range('e'+str(i)).value=element['low']
        prices.range('f'+str(i)).value=element['close']
        prices.range('g'+str(i)).value=element['ltp']
        i=i-1
except Exception as e:
    print("Historic Api failed: {}".format(e.message))

print(type(nifty_100_stocks[i-2]))
